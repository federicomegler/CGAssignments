...
var S3 = `
	ambientColor = ambientLightColor;
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * clamp((dot(normalize(Pos - fs_pos),Dir) - cos(radians(ConeOut/2.0))) / (cos(radians(ConeIn * ConeOut/2.0))-cos(radians(ConeOut/2.0))), 0.0,1.0);
`;
...
var S5 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * pow( (Target / (length(Pos-fs_pos))) , Decay) * clamp((dot(normalize(Pos - fs_pos),Dir) - cos(radians(ConeOut/2.0))) / (cos(radians(ConeIn * ConeOut/2.0))-cos(radians(ConeOut/2.0))), 0.0,1.0);
`;
